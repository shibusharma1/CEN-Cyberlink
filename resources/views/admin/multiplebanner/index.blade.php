...
@section('breadcrumb')
  <a href="{{url('admin/banner')}}" class="btn btn-primary btn-sm">Go Back</a>
					...
					<table class="table admin-form theme-warning fs13">
						...
						<tbody>
							...
							<tr class="id{{$row->id}}">
								...
								<a href="{{url('admin'.'/'.$row->banner_id.'/'.'multiplebanner'.'/'.$row->id.'/edit')}}">Edit</a>
...
// 	url = url.replace(':id',id);
    $.ajax({
      type:'DELETE',
      url:"{{url('admin/'.Request::segment(2).'/multiplebanner') . '/'}}" + id,
