                ...
                @if($data_child->count()>0)
                    @foreach($data_child as $row)
                        @if($row->external_link)
                            ...
                            <span class="uk-display-block uk-text-muted f-14 f-w-400 uk-margin-small-bottom">{{$row->created_at->format('d M Y')}}</span>
                                        <h1 class="f-20 f-w-400 uk-margin-remove"><a  href="{{url(geturl($row['uri'],$row['page_key']))}}">
                                                {{$row->post_title}}</a></h1>
                                    </div>
                                </div>
                            </li>
