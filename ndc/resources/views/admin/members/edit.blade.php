...
<form action="{{ route('member.update', $data->id) }}" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="role" id="role" value="{{$data->role}}" class="gui-input" />     
				...
				<div class="panel panel-warning panel-border top mt20 mb35">
							...
							<div class="section row mb10">
								...
								<div class="col-md-9">
									<label for="first-name" class="field prepend-icon">
										<input type="text" name="first_name" id="first_name" class="gui-input" value="{{$data->first_name}}" />
									</label>
								</div>
							</div>
							...
							<div class="section row mb10">
								...
								<div class="col-md-9">
									<label for="last-name" class="field prepend-icon">
										<input type="text" name="last_name" id="last_name" class="gui-input" value="{{$data->last_name}}" />   
									</label>
								</div>
							</div>
							...
							<div class="section row mb10">
								...
								<div class="col-md-9">
									<label for="account-email" class="field prepend-icon">
										<input type="email" name="email" id="email" class="gui-input" value="{{$data->email}}" />          
									</label>
								</div>
							</div>   
                    ...
                    <div class="col-md-9">
                      <label for="account-phone" class="field prepend-icon">
                        <input type="text" name="phone" id="phone" value="{{$data->phone}}" class="gui-input" />          
                      </label>
                    </div>
                  </div>
                   <div class="section row mb10">
                    ...
                    <div class="col-md-9">
                      <label for="account-designation" class="field prepend-icon">
                        <input type="text" name="designation" id="designation" value="{{$data->designation}}" class="gui-input" />          
                      </label>
                    </div>
                  </div>
						...
						<select name="department" class="empty">
							...
							@foreach($department as $row)
							 <?php //if($row->id == 1){
								...
								//	}
                              ?>
							<option value="{{$row->id}}" {{ ($data->department == $row->id)?'selected':'' }}> {{ ucfirst($row->department) }} </option>
							@endforeach
						</select>
